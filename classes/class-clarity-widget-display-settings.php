...
/**
 * Clarity_Widget_Display_Settings Class
 *
 * @class Clarity_Widget_Display_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	Clarity_Widget_Display
 * @author Jeffikus
 */
final class Clarity_Widget_Display_Settings {
	/**
	...
	/**
	 * Main Clarity_Widget_Display_Settings Instance
	 *
	 * Ensures only one instance of Clarity_Widget_Display_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Clarity_Widget_Display_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true == (bool)apply_filters( 'clarity-widget-display-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'clarity-widget-display-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'clarity-widget-display-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= Clarity_Widget_Display()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'clarity-widget-display-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'clarity-widget-display-newline-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'clarity-widget-display' );
		...
		return (array)apply_filters( 'clarity-widget-display-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				$settings_fields['clarity-widget-code'] = array(
												'name' => __( 'Clarity Widget Code', 'clarity-widget-display' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Your clarity.fm Blog Widget code.', 'clarity-widget-display' )
											);
		...
		return (array)apply_filters( 'clarity-widget-display-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'clarity-widget-display-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'clarity-widget-display-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'clarity-widget-display-' . $v, array() );
